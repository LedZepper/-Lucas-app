  ...
  async function generate(types) {
    ...
    for (let idx = 0; idx < types.length; idx++) {
      ...
      try {
        ...
        const isFracRepr  = st === "fractions_representation";         const isFracEcrit = st === "fractions_ecriture";         const isFracOper  = st === "fractions_operations";         const isFracDecim = st === "fractions_decimales";          if (isFracRepr||isFracEcrit||isFracOper||isFracDecim) { exercises.push({type:st,format:'fractions',title:"Fractions",emoji:"🔢",duration:`${dur} min`,instructions:"Exercice de fractions.",example:"",lignes:["En cours de développement"],parentNote:"",verbsUsed:[],wordsUsed:[]}); continue; } if (isFracRepr) {           const CONFIGS_REPR = [             {total:4,filled:1},{total:4,filled:3},{total:6,filled:2},{total:6,filled:4},             {total:8,filled:3},{total:8,filled:5},{total:3,filled:1},{total:3,filled:2},             {total:5,filled:2},{total:5,filled:3},{total:10,filled:4},{total:10,filled:7},           ];           const EQUIV_PAIRS = [["1/2","2/4"],["2/4","4/8"],["1/3","2/6"],["2/3","4/6"],["1/4","2/8"],["3/4","6/8"],["1/5","2/10"],["2/5","4/10"]];           const COMPARE_PAIRS = [["1/4","3/4"],["2/3","1/3"],["1/2","3/4"],["3/8","5/8"],["1/5","4/5"],["2/6","5/6"],["1/3","2/3"],["3/10","7/10"]];           const FRACS_Q5 = ["1/2","1/3","1/4","2/3","3/4","1/5","2/5","3/5","4/5","1/8","3/8","5/8"];           const sh = [...CONFIGS_REPR].sort(()=>Math.random()-.5);           const [cfg1,cfg2] = sh;           const equiv = EQUIV_PAIRS[Math.floor(Math.random()*EQUIV_PAIRS.length)];           const compare = COMPARE_PAIRS[Math.floor(Math.random()*COMPARE_PAIRS.length)];           const fracQ5 = FRACS_Q5[Math.floor(Math.random()*FRACS_Q5.length)];           const lignes = [             `1. Quelle fraction représente la partie coloriée ? SQUARES:${cfg1.total}:${cfg1.filled} → _______________`,             `2. Quelle fraction représente la partie coloriée ? SQUARES:${cfg2.total}:${cfg2.filled} → _______________`,             `3. Ces deux fractions sont-elles égales ? ${equiv[0]} et ${equiv[1]} → _______________`,             `4. Entoure la plus grande fraction : ${compare[0]}   ou   ${compare[1]}`,             `5. Colorie ${fracQ5} de cette bande : SQUARES:${fracQ5.split("/")[1]}:0 → _______________`,           ];           exercises.push({type:st,format:'fractions',title:"Représenter une fraction",emoji:"🔢",duration:`${dur} min`,instructions:"Réponds à chaque question sur les fractions.",example:"Rappel : dans 3/4, le 4 = nombre de parts totales, le 3 = parts prises.",lignes,parentNote:"",verbsUsed:[],wordsUsed:[]});           continue;         }          if (isFracEcrit) {           const CONTEXTES = [             {enonce:"Un ruban de 12 cm est coupé en 4 parts égales.",q:"Quelle fraction représente 3 parts ?"},             {enonce:"Une pizza partagée en 8 parts. Tom mange 3 parts.",q:"Quelle fraction Tom a-t-il mangée ?"},             {enonce:"Un trajet de 20 km. Marie a parcouru 15 km.",q:"Quelle fraction du trajet a-t-elle parcouru ?"},             {enonce:"Un chocolat divisé en 6 carreaux. On mange 2 carreaux.",q:"Quelle fraction reste-t-il ?"},             {enonce:"Une journée de 24 heures. Léo dort 8 heures.",q:"Quelle fraction de la journée Léo dort-il ?"},             {enonce:"Un carré partagé en 5 parties. On colorie 2 parties.",q:"Quelle fraction est coloriée ?"},             {enonce:"Une boîte de 10 billes. 3 billes sont bleues.",q:"Quelle fraction est bleue ?"},             {enonce:"Un livre de 30 pages. Clara a lu 10 pages.",q:"Quelle fraction Clara a-t-elle lue ?"},           ];           const sh = [...CONTEXTES].sort(()=>Math.random()-.5).slice(0,3);           const lignes = sh.flatMap((c,i)=>[`${i+1}. ${c.enonce} ${c.q}`,`   Réponse : _______________`,""]).slice(0,-1);           exercises.push({type:st,format:'fractions',title:"Fractionner les parts",emoji:"🔢",duration:`${dur} min`,instructions:"Lis chaque problème et écris la fraction demandée.",example:"Rappel : 3 parts sur 8 = 3/8",lignes,parentNote:"",verbs
